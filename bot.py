...
def parse_prop_request(message_text):
    ...
    stat = 'points'
    if any(x in text for x in ['pra', 'points rebounds assists']):
        stat = 'points rebounds assists'
    elif any(x in text for x in ['p+a', 'points assists']):
        stat = 'points assists'
    elif any(x in text for x in ['p+r', 'points rebounds']):
        stat = 'points rebounds'
    elif any(x in text for x in ['3pm', 'three pointer', 'threes', '3 pointer']):
...
def analyze_prop(player, stat, line):
    ...
    user = (
        'Analyze this NBA prop bet. Player: ' + player +
        ' Stat: ' + stat +
        ' Line: ' + str(line) +
        ' Today: ' + today +
        ' Find their team and opponent today. Give last 10 game logs for this stat.'
        ' Give last 5 H2H games vs today opponent. Check injury status.'
        ' Project the stat and give a verdict.'
        ' Return ONLY this JSON object:'
        ' {"player_name": "string", "team": "string", "position": "string",'
        ' "opponent": "string", "game_time": "string", "stat_type": "' + stat + '",'
        ' "line": ' + str(line) + ', "season_avg": 0.0, "last10_avg": 0.0,'
        ' "last10_games": [{"game": "vs/@ TEAM Mon DD", "value": 0}],'
        ' "h2h_avg": 0.0, "h2h_games": [{"game": "vs/@ TEAM Mon DD", "value": 0}],'
        ' "injury_status": "Active", "projected": 0.0, "confidence": "HIGH",'
        ' "summary": "1-2 sentence summary", "hit_rate_over": 0,'
        ' "over_points": ["reason1", "reason2", "reason3"],'
        ' "under_points": ["reason1", "reason2", "reason3"],'
        ' "verdict": "OVER", "verdict_explanation": "2-3 sentences"}'
    )
    raw = gemini_chat(system, user)
...
def build_embed(d, line):
    ...
    embed = discord.Embed(
        title=str(d.get('player_name', 'Player')) + '  -  ' + str(d.get('stat_type', '')) + ' O/U ' + str(line),
        description=(
    ...
    embed.set_footer(text='NBA Prop Analyst - Powered by Gemini - For entertainment only')
    return embed
...
HELP_TEXT = (
    ...
    '> `@Propbot Jaylen Brown 21.5 points`\n\n'
...
async def on_message(message):
    ...
    async with message.channel.typing():
        ...
        if not prop:
            await message.reply('Could not parse that prop. Try: `@Propbot LeBron James over 25.5 points`')
            return
        player = prop['player']
        stat = prop['stat']
        line = prop['line']
        ...
        try:
            ...
            if not data or not data.get('player_name'):
                await thinking.edit(content='Could not generate analysis for **' + player + '**. Try rephrasing.')
                return
            embed = build_embed(data, line)
            await thinking.edit(content='', embed=embed)
        except urllib.error.HTTPError as e:
            body = e.read().decode()
            await thinking.edit(content='API error ' + str(e.code) + ': ' + body[:150])
...
def analyze_prop(player, stat, line):
    today = datetime.now().strftime('%A, %B %d, %Y')
    prompt = (
        'You are an NBA prop analyst. Today: ' + today + '.'
        ' Analyze: Player=' + player +
        ' Stat=' + stat +
        ' Line=' + str(line) +
        ' Return ONLY valid JSON, no markdown, no extra text:'
        ' {"player_name":"string","team":"string","position":"string",'
        '"opponent":"string todays opponent","game_time":"string",'
        '"stat_type":"' + stat + '","line":' + str(line) + ','
        '"season_avg":0.0,"last10_avg":0.0,'
        '"last10_games":[{"game":"vs TEAM Mon DD","value":0}],'
        '"h2h_avg":0.0,"h2h_games":[{"game":"vs TEAM Mon DD","value":0}],'
        '"injury_status":"Active","projected":0.0,'
        '"confidence":"HIGH","summary":"brief summary",'
        '"hit_rate_over":0,'
        '"over_points":["r1","r2","r3"],'
        '"under_points":["r1","r2","r3"],'
        '"verdict":"OVER","verdict_explanation":"explanation"}'
    )
...
def build_embed(d, line):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            '  vs  ' + str(d.get('opponent', '?')) +
            '  ' + str(d.get('game_time', 'Today')) +
            '  |  ' + str(d.get('injury_status', 'Active'))
        ),
        colour=colour,
        timestamp=datetime.utcnow()
    )
...
async def on_message(message):
    ...
    async with message.channel.typing():
        ...
        try:
            ...
            body = e.read().decode()
            await thinking.edit(content='API error ' + str(e.code) + ': ' + body[:200])
        except Exception as e:
            await thinking.edit(content='Error: ' + str(e)[:200])
