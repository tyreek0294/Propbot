...
def analyze_prop(player, stat, line):
    ...
    system = 'You are an NBA prop analyst with accurate knowledge of the current NBA 2025-26 season schedule, team rosters, and player stats. You must identify the player\'s ACTUAL scheduled opponent for today ' + today + ' based on the real NBA schedule - do not guess or use a random opponent. Reply with ONLY raw JSON, no markdown, no explanation.'
    user = (
        'Analyze this NBA prop bet for today ' + today + '.'
        ' Player: ' + player + '. Stat: ' + stat + '. Line: ' + str(line) + '.'
        ' IMPORTANT: First identify which team ' + player + ' plays for and who they play TODAY within the next 24 hours on ' + today + '. Use that as the opponent for H2H stats.'
        ' Do NOT use a random opponent - it must be their actual game today.'
        ' Provide their real last 10 game logs for ' + stat + ' with actual dates and realistic values.'
        ' Provide last 5 H2H games specifically vs TODAY opponent with realistic values.'
        ' For over_points and under_points each provide exactly 3 detailed one-sentence reasons.'
        ' Each reason must reference specific stats - e.g. mention the last 10 avg, H2H avg, defensive rating, pace, injury context.'
        ' hit_rate_over = how many of the last 10 games went over ' + str(line) + '.'
        ' Return ONLY this JSON:'
        ' {"player_name":"","team":"","position":"","opponent":"ACTUAL todays opponent team name","game_time":"time ET",'
        ...
        '"injury_status":"Active or detail","projected":0.0,"confidence":"HIGH or MEDIUM or LOW",'
        '"summary":"2 sentence summary mentioning key stats",'
        '"hit_rate_over":0,'
        '"over_points":["Detailed one-sentence reason referencing specific stats","Detailed one-sentence reason referencing H2H or matchup","Detailed one-sentence reason referencing form or context"],'
        '"under_points":["Detailed one-sentence reason referencing specific stats","Detailed one-sentence reason referencing H2H or defense","Detailed one-sentence reason referencing risk factors"],'
        '"verdict":"OVER or UNDER or LEAN OVER or LEAN UNDER or PASS",'
        '"verdict_explanation":"2-3 sentence explanation citing the key stats that drove the verdict"}'
    )
